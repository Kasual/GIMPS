...
"  fabs				#17     err,NTEMP0                     \n"  \
"  fldl %4			#18     maxerr,err,NTEMP0              \n"  \
"  fcomi %%st(1)                #19-20  maxerr,err,NTEMP0              \n"  \
"  fld  %%st(2)			#19     temp0,maxerr,err,temp0         \n"  \
"  faddl 32(%%esi)		#20-22	TEMP0+BIGA,maxerr,err,temp0    \n"  \
"  fxch %%st(1)			#20	maxerr,TEMP0+BIGA,err,temp0    \n"  \
"  fcmovb %%st(2),%%st(0)       #21     maxerr,TEMP0+BIGA,err,temp0    \n"  \
"  fldl 8(%%edi)		#22	TT[1],maxerr,TEMP0+BIGA,err,temp0 \n"\
"  fmull 16(%%esi)		#23-25  ttmpBig*TT[1]=TT[1],maxerr,temp0+biga,err,temp0\n"\
"  fxch %%st(2)			#23	temp0+biga,maxerr,TT[1],err,temp0\n"\
"  fsubl 32(%%esi)		#24-26	NEWCARRY,maxerr,TT[1],err,temp0 \n" \
"  fxch  %%st(1)		#25     maxerr,NEWCARRY,TT[1],err,temp0 \n" \
"  fstp  %%st(3)                #26     NEWCARRY,tt[1],maxerr,temp0    \n"  \
"  fstl %3			#27	newcarry,tt[1],maxerr,temp0    \n"  \
"  fsubrp %%st(0),%%st(3)	#29-31  tt[1],maxerr,TEMP0-NEWCARRY    \n"  \
"  fldl (%%edi)			#30	tt[0],tt[1],maxerr,TEMP0-NEWCARRY \n"\
"  fmul %%st(0),%%st(3)         #31-32  TT[0],tt[1],maxerr,NEWDATA     \n"  \
"  fxch %%st(1)                 #31     tt[1],tt[0],maxerr,NEWDATA     \n"  \
"  fstpl 8(%%edi)               #32     tt[0],maxerr,NEWDATA           \n"  \
"  fmull 24(%%esi)              #33-37  TT[0],maxerr,NEWDATA           \n"  \
"  fxch %%st(1)                 #33     maxerr,TT[0];NEWDATA           \n"  \
"  fstpl %4                     #34     TT[0],newdata                  \n"  \
"  fxch %%st(1)                 #34     newdata,TT[0]                  \n"  \
"  fstpl %2                     #35     TT[0]                          \n"  \
"  fstpl (%%edi)                #36                                    \n"  \
