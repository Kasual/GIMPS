...
#define init_twiddle_factors_14\
  tw1r=1.0; tw1i=0.0;\
  tw2r=tw1r; tw2i=tw1i;\
  tw3r=tw1r; tw3i=tw1i;\
  tw4r=tw1r; tw4i=tw1i;\
...
# define  load_two_to_minusphi_14(_i) \
	tt0[1] = two_to_minusphi[addr(_i)]; \
	tt1[1] = two_to_minusphi[addr(((pad2>>SHIFT_UPDATE) + _i))];\
	tt2[1] = two_to_minusphi[addr(((pad4>>SHIFT_UPDATE) + _i))];\
	tt3[1] = two_to_minusphi[addr(((pad6>>SHIFT_UPDATE) + _i))];\
...
#define radix_14_twd_last_dit(_j) \
	  ...
	  cplx_fft7(B,t7,t8,t9,t10,t11,t12,t13);\
	   \
	  cplx_addsub(t0,t7);\
	  cplx_muladdsub(t1,t8, B_1_14);\
          cplx_muladdsub(t2,t9, B_1_7);\
	  cplx_muladdsub(t3,t10, B_3_14);\
	  cplx_muladdsub(t4,t11, B_2_7);\
	  cplx_muladdsub(t5,t12, B_5_14);\
	  cplx_muladdsub(t6,t13, B_3_7);
...
#define radix_14_first_dif(_j) \
	  cplx_fft7(F,t0,t2,t4,t6,t8,t10,t12);\
	  cplx_fft7(F,t1,t3,t5,t7,t9,t11,t13);\
	   \
	  cplx_addsub_store_p(_j,pd0,pd7,t0,t1);\
	  cplx_muladdsub_store_p(_j,pd1,pd8,t2,t3, F_1_14);\
	  cplx_muladdsub_store_p(_j,pd2,pd9,t4,t5, F_1_7);\
	  cplx_muladdsub_store_p(_j,pd3,pd10,t6,t7, F_3_14);\
	  cplx_muladdsub_store_p(_j,pd4,pd11,t8,t9, F_2_7);\
	  cplx_muladdsub_store_p(_j,pd5,pd12,t10,t11, F_5_14);\
	  cplx_muladdsub_store_p(_j,pd6,pd13,t12,t13, F_3_7);


#define radix_14_first_dif_add(_d)               \
	  cplx_fft7(F,t0,t2,t4,t6,t8,t10,t12);   \
	  cplx_fft7(F,t1,t3,t5,t7,t9,t11,t13);   \
	                                         \
	  cplx_addsub(t0,t1);                    \
	  cplx_local_to_data_add(_d, 0, t0);     \
	  cplx_local_to_data_add(_d, pad7, t1);  \
	  cplx_muladdsub(t2,t3, F_1_14);         \
	  cplx_local_to_data_add(_d, pad, t2);   \
	  cplx_local_to_data_add(_d, pad8, t3);  \
	  cplx_muladdsub(t4,t5, F_1_7);          \
	  cplx_local_to_data_add(_d, pad2, t4);  \
	  cplx_local_to_data_add(_d, pad9, t5);  \
	  cplx_muladdsub(t6,t7, F_3_14);         \
	  cplx_local_to_data_add(_d, pad3, t6);  \
	  cplx_local_to_data_add(_d, pad10, t7); \
	  cplx_muladdsub(t8,t9, F_2_7);          \
	  cplx_local_to_data_add(_d, pad4, t8);  \
	  cplx_local_to_data_add(_d, pad11, t9); \
	  cplx_muladdsub(t10,t11, F_5_14);       \
	  cplx_local_to_data_add(_d, pad5, t10); \
	  cplx_local_to_data_add(_d, pad12, t11);\
	  cplx_muladdsub(t12,t13, F_3_7);        \
	  cplx_local_to_data_add(_d, pad6, t12); \
	  cplx_local_to_data_add(_d, pad13, t13);

#define radix_14_first_dif_add_shift(_d,_ofs)    \
	  cplx_fft7(F,t0,t2,t4,t6,t8,t10,t12);   \
	  cplx_fft7(F,t1,t3,t5,t7,t9,t11,t13);   \
	                                         \
	  cplx_addsub(t0,t1);                    \
...
void substract_two_14(BIG_DOUBLE *x, UL N)
{
  ...
  radix_14_first_dif_add_shift(x,ofs);
...
#  else
  ...
  ttmp9,ttmp10,ttmp11,ttmp12,ttmp13;
...
#  endif
  ...
  BIG_DOUBLE carry0=0.0,carry1=0.0,carry2=0.0,carry3=0.0,
                                          carry4=0.0,carry5=0.0,carry6=0.0,carry7=0.0,carry8=0.0,carry9=0.0,
                                                                                  carry10=0.0,carry11=0.0,carry12=0.0,carry13=0.0;
  ...
  BIG_DOUBLE *px;
  UL bj0,bj1,bj2,bj3,bj4,bj5,bj6,bj7,bj8,bj9,bj10,bj11,bj12,bj13;
  ...
  for (i=0,j=0; j<pad2; j+=UPDATE,i++)
    {
      load_dwf_14 (i);
      /*
      load_two_to_minusphi_14(i);
      load_two_to_phi_14(i);*/
...
#  endif
              ...
              cplx_carry_norm_last_check(t,13);
            }
          else
            {
#  if defined(Y_VECTORIZE)
...
#  endif
              ...
              cplx_carry_norm_last(t,13);
            }
          radix_14_first_dif( ll );
...
# if defined(Y_MINIMUM)
#  define init_twiddle_factors_14_mp(_px)  \
...
# define save_carries_14_mp(_pcarr)  \
        _pcarr[0]=carry0;            \
        _pcarr[1]=carry1;            \
        _pcarr[2]=carry2;            \
        _pcarr[3]=carry3;            \
...
#define radix_14_first_dif_add_mp \
	  cplx_fft7(F,t0,t2,t4,t6,t8,t10,t12);\
	  cplx_fft7(F,t1,t3,t5,t7,t9,t11,t13);\
	   \
	  cplx_addsub(t0,t1);\
	  cplx_local_to_data_add(pd0, 0, t0);\
	  cplx_local_to_data_add(pd7, 0, t1);\
	  cplx_muladdsub(t2,t3, F_1_14);\
	  cplx_local_to_data_add(pd1, 0, t2);\
	  cplx_local_to_data_add(pd8, 0, t3);\
	  cplx_muladdsub(t4,t5, F_1_7);\
	  cplx_local_to_data_add(pd2, 0, t4);\
	  cplx_local_to_data_add(pd9, 0, t5);\
	  cplx_muladdsub(t6,t7, F_3_14);\
	  cplx_local_to_data_add(pd3, 0, t6);\
	  cplx_local_to_data_add(pd10, 0, t7);\
	  cplx_muladdsub(t8,t9, F_2_7);\
	  cplx_local_to_data_add(pd4, 0, t8);\
	  cplx_local_to_data_add(pd11, 0, t9);\
	  cplx_muladdsub(t10,t11, F_5_14);\
	  cplx_local_to_data_add(pd5, 0, t10);\
	  cplx_local_to_data_add(pd12, 0, t11);\
	  cplx_muladdsub(t12,t13, F_3_7);\
	  cplx_local_to_data_add(pd6, 0, t12);\
	  cplx_local_to_data_add(pd13, 0, t13);


BIG_DOUBLE dit_carry_norm_dif_14_mp(BIG_DOUBLE *x,BIG_DOUBLE *ptx, BIG_DOUBLE *pcarr, UL *bp, UL N ,UL n0, UL nthr,UL err_flag)
{
#  ifdef Y_KILL_BRANCHES
  double Y_ALIGNED(16) aux[6],maxerr=0.0;
  double Y_ALIGNED(16) tt0[2],tt1[2],tt2[2],tt3[2],tt4[2],tt5[2],tt6[2],tt7[2],tt8[2],
  ...
  if(nthr)
    {
      init_bjs_14_mp;
      get_pads_14;
      init_twiddle_factors_14_mp(ptx);
      px = ptx + Y_STEP;
    }
  else
    {
...
# endif
  ...
  /* save the carries */
  save_carries_14_mp(pcarr);

  /* return the error */
  return maxerr;
}

void dit_carry_norm_dif_14_last_carries_mp(BIG_DOUBLE *x, UL *bp, UL N ,UL n0, int nthr)
{
#  ifdef Y_KILL_BRANCHES
...
#  endif
  ...
  carry9,carry10,carry11,carry12,carry13;
  BIG_DOUBLE t0r,t0i,t1r,t1i,t2r,t2i,t3r,t3i,t4r,t4i,t5r,t5i,t6r,t6i,
  t7r,t7i,t8r,t8i,t9r,t9i,t10r,t10i,t11r,t11i,t12r,t12i,t13r,t13i;
  UL bj0,bj1,bj2,bj3,bj4,bj5,bj6,bj7,bj8,bj9,bj10,bj11,bj12,bj13;
  y_ptr pd0,pd1,pd2,pd3,pd4,pd5,pd6,pd7,pd8,pd9,pd10,pd11,pd12,pd13;
...
# endif
  ...
  for(i=0;i<Y_NTHREADS;i++)
    {
      Y_ERRS[i]=dit_carry_norm_dif_14_mp( x, Y_TWX[i], Y_CARRIES[i], Y_BJS[i],
                                          N, n0, i, err_flag);
    }
...
# endif
  for(i=0;i<Y_NTHREADS;i++)
    {
      dit_carry_norm_dif_14_last_carries_mp( x, Y_BJS[i],
                                             N, n0, i);
    }
...
void dit_carry_norm_dif_14(BIG_DOUBLE *x,UL N, UL err_flag )
{
  printf("Called dit_carry_norm_dif_14 , x=%p, N=%ld, Err_flag=%ld\n",x,N,err_flag);
  printf("Please compile again the file ynorm_14.c with \n -DY_AVAL=4 -DY_MANY_REGISTERS flags\n");
  exit(-1);
}
