...
#define init_bjs_7                       \
  ...
  Y_MM_SET_PD( bj6x, bj6,    0);

#define round_bjs_7                          \
  Y_MM_TWO_RINT_PD( bj03, bj14, bj03, bj14); \
  Y_MM_TWO_RINT_PD( bj25, bj6x, bj25, bj6x);

#define  sse2_load_dwt_7(_i)                                            \
  {                                                                     \
    size_t j0, j1;                                                      \
    j0 = addr (_i);                                                     \
    ...
    prefetch_data_trig (two_to_phi, j0 + Y_CACHE_LINE);                 \
    prefetch_data_trig (two_to_phi, j1 + Y_CACHE_LINE);                 \
    j0 = addr (((pad6 >> (SHIFT_UPDATE - 1)) + _i));                    \
    Y_MM_SET_PD (ttmp6x, two_to_minusphi[j0], 0.0);                     \
    Y_MM_SET_PD (ttp6x, two_to_phi[j0], 0.0);                           \
    prefetch_data_trig (two_to_minusphi, j0 + Y_CACHE_LINE);            \
    prefetch_data_trig (two_to_phi, j0 + Y_CACHE_LINE);                 \
  }
...
#define inter_chain_7              \
    ...
    Y_MM_MOV_INTER_PAIR(t6r, t7r); \
    Y_MM_MOV_INTER_PAIR(t6i, t7i);
...
# define init_twiddle_factors_sse2_7( _pd )      \
...
# define radixmm_7_twd_last_dit(_j)                          \
  ...
  sse2_fft7 (B, t0, t1, t2, t3, t4, t5, t6);                 \
  Y_MM_SETZERO_PD( t7r);                                     \
  Y_MM_SETZERO_PD( t7i);                                     \
  inter_chain_7;
...
# define radixmm_7_first_dif(_j)                             \
  inter_chain_7;                                             \
  sse2_fft7_store_inter_p (_j, pd0, pd1, pd2, pd3, pd4, pd5, pd6, F, t0, t1, t2, t3, t4, t5, t6);


#define radixmm_7_first_dif_add(_d)           \
  inter_chain_7;                              \
  sse2_fft7 (F, t0, t1, t2, t3, t4, t5, t6);  \
  ...
  sse2_local_to_data_inter_add (_d, pad6, t6);

#define radixmm_7_first_dif_add_shift(_d, _ofs)\
  sse2_fft7 (F, t0, t1, t2, t3, t4, t5, t6);   \
...
# define radixmm_7_first_dif(_j)                             \
  inter_chain_7;                                             \
  sse2_fft7_store_p (_j, pd0, pd1, pd2, pd3, pd4, pd5, pd6, F, t0, t1, t2, t3, t4, t5, t6);
...
#define radixmm_7_first_dif_add(_d)         \
  ...
  sse2_local_to_data_add (_d, pad6, t6);
...
void substract_two_7_sse2(BIG_DOUBLE *x, UL N)
{
  ...
  */
  ...
  radixmm_7_first_dif_add_shift (x, ofs);
...
void dit_carry_norm_dif_7_sse2 (BIG_DOUBLE *x, UL N , UL err_flag)
{
  Y__M128D ttp03, ttp14, ttp25, ttp6x, ttmp03, ttmp14, ttmp25, ttmp6x;
  Y__M128D tw1r, tw1i, tw2r, tw2i, tw3r, tw3i, tw4r, tw4i, tw5r, tw5i,
  tw6r, tw6i;
  Y__M128D t0r, t0i, t1r, t1i, t2r, t2i, t3r, t3i, t4r, t4i, t5r, t5i,
  t6r, t6i, t7r, t7i;
  Y__M128D bj03, bj14, bj25, bj6x;
  Y__M128D carry03, carry14, carry25, carry6x, maxerr;
#if defined(SUM_CHECK)
  ...
  y_ptr px, pd0, pd1, pd2, pd3, pd4, pd5, pd6;
  UL pad = Y_LRIGHT[1], pad2, pad3, pad4, pad5, pad6;
  UL i, j, k, l, ll;
  ...
  init_twiddle_factors_sse2_7 (px);
          ...
          if(err_flag)
            {
              ...
              cplx_carry_norm_check_sse2 (t5i, 25);
              cplx_carry_norm_check_sse2_sd (t6r, 6x);
              cplx_carry_norm_check_sse2_sd (t6i, 6x);
              cplx_carry_norm_check_sse2_sd (t7r, 6x);
              cplx_carry_norm_check_sse2_last_odd_sd (t7i, 6x);
            }
          else
            {
#if defined(Y_VECTORIZE)
              ...
              cplx_carry_norm_sse2 (t5i, 25);
              cplx_carry_norm_sse2_sd (t6r, 6x);
              cplx_carry_norm_sse2_sd (t6i, 6x);
              cplx_carry_norm_sse2_sd (t7r, 6x);
              cplx_carry_norm_sse2_last_odd_sd (t7i, 6x);
            }
          radixmm_7_first_dif( ll );
  ...
  /* adjust the last carries */
  {
    ...
    Y_MM_STORE_PD( aux, carry25);
    Y_MM_STORE_PD( aux + 2, carry6x);
    aux0   = aux[1];
    aux[1] =    0.0;
    aux[3] = aux[0];
    aux[0] =   aux0;
    Y_MM_LOAD_PD( t6r, aux);
  ...
  prefetch_data( pd2, 0);
  prefetch_data( pd5, 0);
  cplx_carry_sse2(t6r,6x);
  cplx_carry_sse2(t6i,6x);
  cplx_carry_sse2(t7r,6x);
  cplx_carry_sse2(t7i,6x);
  prefetch_data( pd6, 0);

  /* add carry signal */
  radixmm_7_first_dif_add( x );
...
#  define init_twiddle_factors_sse2_7_mp(_px)              \
...
# define save_carries_7_mp(_pcarr)          \
        ...
        Y_MM_STORE_PD( _pcarr + 6, carry6x);\
        _pcarr[7] = 0.0;

# if !defined(ALL_INTERLACED)
#  define radixmm_7_first_dif_add_mp       \
  inter_chain_7;                           \
  sse2_fft7 (F, t0, t1, t2, t3, t4, t5, t6);\
  ...
  sse2_local_to_data_inter_add (pd6, 0, t6);
...
#  define radixmm_7_first_dif_add_mp        \
  ...
  sse2_local_to_data_add (pd6, 0, t6);

# endif

BIG_DOUBLE dit_carry_norm_dif_7_mp_sse2(y_ptr x, y_ptr ptx, y_ptr pcarr,
                                        y_ptr bp, UL N ,UL n0, UL nthr, UL err_flag)
{
  ...
  Y__M128D bj03, bj14, bj25, bj6x;
  Y__M128D carry03, carry14, carry25, carry6x, maxerr ;
  y_ptr pd0, pd1, pd2, pd3, pd4, pd5, pd6, px;
  UL pad = Y_LRIGHT[1], pad2, pad3, pad4, pad5, pad6;
  ...
  if(nthr)
    {
      ...
      */
      init_twiddle_factors_sse2_7_mp (ptx);
      px = ptx + 2 * Y_STEP;
    }
  else
    {
      ...
      */
      if(Y_SBIT == 0)
        Y_MM_SET_PD(carry03, -2.0, 0.0);
      init_twiddle_factors_sse2_7 (ptx);
      px = ptx + Y_STEP;
    }
...
# ifdef SUM_CHECK
          ...
          sse2_sum_7 (Y_SUMOUT[nthr]);
# endif

          /*if( (k + j + 4 ) < lim ) get_twiddle_factors_sse2_7;*/
          if(err_flag)
            {
...
void dit_carry_norm_dif_7_last_carries_mp_sse2 (y_ptr x, y_ptr bp, UL N ,UL n0, int nthr)
{
  Y__M128D ttp03, ttp14, ttp25, ttp6x, ttmp03, ttmp14, ttmp25, ttmp6x;
  Y__M128D t0r, t0i, t1r, t1i, t2r, t2i, t3r, t3i, t4r, t4i, t5r, t5i,
  t6r, t6i, t7r, t7i;
  Y__M128D bj03, bj14, bj25, bj6x;
  Y__M128D carry03, carry14, carry25, carry6x;
  ...
  if(nthr)
    {
      ...
      Y_MM_LOAD_PD( t6r, Y_CARRIES[nthr - 1] + 6);
    }
  else
    {
      double Y_ALIGNED(16) aux[4];
      aux[0] = Y_CARRIES[Y_NTHREADS - 1][5];
      aux[1] = 0.0;
      aux[2] = Y_CARRIES[Y_NTHREADS - 1][6];
      aux[3] = Y_CARRIES[Y_NTHREADS - 1][4];
      Y_MM_LOAD_PD( t6r, aux);
      Y_MM_LOAD_PD( t0r, aux + 2);
...
# endif
  ...
  for(i = 0; i < Y_NTHREADS; i++)
    {
      Y_ERRS[i] = dit_carry_norm_dif_7_mp_sse2 ( x, Y_TWX[i], Y_CARRIES[i], Y_XBJS[i],
                  N, n0, i, err_flag);
    }
...
# endif
  for(i = 0; i < Y_NTHREADS; i++)
    {
      dit_carry_norm_dif_7_last_carries_mp_sse2( x, Y_XBJS[i],
          N, n0, i);
    }
...
{
  printf("The routine void_carry_norm_7_sse2 should never be called\n");
  exit(EXIT_FAILURE);
}
